...
import {NavItem} from './Navigation-slider';
...
class Navigation extends Component {
  ...
  componentDidMount() {
    ...
    .then(entry => this.setState({navigation: entry.fields.link}))
  ...
  render() {
    return (
      ...
      <div id="navigation-contianer">
        ...
        <div className={`navigation-slider ${this.state.nav_state ? " open" : ""}`}>
        <div className="navigation-items">
          <div className="navigation-header">
            <NavItem title="Montague" url="/"></NavItem>
          </div>
          
            {this.state.navigation.map(item => {
              const {title, url} = item.fields
              return (
                <NavItem onClickHanlder={this.onClickHanlder} title={title} url={url} parent="True"></NavItem>
              )
            })}
          </div>
        </div>
